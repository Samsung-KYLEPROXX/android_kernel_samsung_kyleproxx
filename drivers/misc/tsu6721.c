...
unsigned int musb_get_charger_type(void)
{
	...
	value2 = i2c_smbus_read_byte_data(client, REG_DEVICE_TYPE2);
	value3 = i2c_smbus_read_byte_data(client, REG_DEVICE_TYPE3);
	printk("%s chrgr type1 0x%x\n", __func__, value);
	printk("%s chrgr type2 0x%x\n", __func__, value2);
	printk("%s vbus debounce 0x%x\n", __func__, value3);

	if(value & DEV_USB || value2 & DEV_T2_USB_MASK || value & DEV_USB_CHG)
	{
		return POWER_SUPPLY_TYPE_USB;
	}
	else if(value & DEV_T1_CHARGER_MASK || value3 & DEV_T3_CHARGER_MASK)
	{
		return POWER_SUPPLY_TYPE_USB_DCP;
	}
	else if(value3 & DEV_VBUS_DEBOUNCE)
	{
...
u16 tsu6721_get_chrgr_type(void)
{		
	if( !local_usbsw ){
		printk("failed to allocate driver data\n");
		return 0;
