...
static int rt8973_get_cable_type_by_id(rt8973_chip_t *chip)
{
    ...
    /* Special case for OTG removal bug; cable type 5 */
    if (chip->prev_status.cable_type == MUIC_RT8973_CABLE_TYPE_OTG) {
        if (cable_type == MUIC_RT8973_CABLE_TYPE_ATT_TA)
            cable_type = MUIC_RT8973_CABLE_TYPE_UNKNOWN;
    }
...
static void rt8973_preprocess_status(rt8973_chip_t *chip)
{
	...
	chip->prev_status.cable_type = chip->curr_status.cable_type;
...
static void rt8973_init_work(struct work_struct *work)
{
	...
	queue_delayed_work(chip->wq, &chip->irq_work, msecs_to_jiffies(200));
}
