...
#else
#ifdef CONFIG_FLASH_SKY81290
static int camdrv_ss_s5k4ecgx_SKY81290_flash_control(struct v4l2_subdev *sd, int control_mode);
#else
...
#else
#ifdef CONFIG_FLASH_SKY81290
#define camdrv_ss_s5k4ecgx_flash_control(a,b)  camdrv_ss_s5k4ecgx_SKY81290_flash_control(a,b)
#else
...
|| defined(CONFIG_MACH_JAVA_SS_BAFFINLITE_REV00)|| defined(CONFIG_MACH_JAVA_SS_BAFFINLITE_REV02)\
...
#ifdef CONFIG_FLASH_SKY81290
static void camdrv_ss_SKY81290_flash_write_data(char addr, char data)
{
int i;
CAM_INFO_PRINTK( "%s %d %d:  E\n", __func__, addr,data);
    /* send address */
...
#if !defined(CONFIG_MACH_HAWAII_SS_KYLEPRO) && !defined(CONFIG_MACH_HAWAII_SS_KYLEPRODS) 
		CAM_INFO_PRINTK("%s , ae unlock\n ",__func__);
		err = camdrv_ss_i2c_set_config_register(client,s5k4ecgx_ae_unlock_regs, ARRAY_SIZE(s5k4ecgx_ae_unlock_regs), "ae_unlock_regs");
//		CAM_INFO_PRINTK("%s , awb unlock\n ",__func__);
		if (state->currentWB== WHITE_BALANCE_AUTO)
...
/*optional*/
	...
	#else
	#ifdef CONFIG_FLASH_SKY81290
	sensor->flash_control    	   = camdrv_ss_s5k4ecgx_SKY81290_flash_control;
	#else
