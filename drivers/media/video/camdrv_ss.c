...
static int camdrv_ss_actual_sensor_power_up(void);
...
static reg_hash_t reg_hash_table[] = {
	...
	{"init_regs_smart_stay",						NULL},
	 ...
	 || defined(CONFIG_MACH_HAWAII_SS_GOLDENVEN) || defined(CONFIG_MACH_HAWAII_SS_KYLEVE)\
	 || defined(CONFIG_MACH_HAWAII_SS_CS02)	|| defined(CONFIG_MACH_HAWAII_SS_HEAT)\
...
#if defined( CONFIG_MACH_HAWAII_SS_KYLEPRO) || defined(CONFIG_MACH_HAWAII_SS_KYLEPRODS) 
	CAM_INFO_PRINTK("switch mode =%d\n", state->mode_switch);
	if(state->mode_switch==CAMCORDER_PREVIEW_TO_CAMERA_PREVIEW){
			CAM_INFO_PRINTK("%s , ae unlock\n ",__func__);
			err = camdrv_ss_i2c_set_config_register(client,sensor.ae_unlock_regs, sensor.rows_num_ae_unlock_regs, "ae_unlock_regs");
	//		CAM_INFO_PRINTK("%s , awb unlock\n ",__func__);
			if (state->currentWB== WHITE_BALANCE_AUTO)
...
static int camdrv_ss_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	{
		if (atomic_read(&gCapModeState) == CAMDRV_SS_CAPTURE_MODE_NOT_SUPPORT)
			ctrl->value = 0;
		else if (atomic_read(&gCapModeState) ==
