...
static int __devinit fc8150_spi_probe(struct spi_device *spi)
{
	...
	PRINTF(0, "(SJINU) fc8150_spi_probe\n");

	spi->max_speed_hz =  6500000;
	spi->bits_per_word = 8;
	spi->mode =  SPI_MODE_0;
	...
	, u8 *txbuf, u16 tx_length, u8 *rxbuf, u16 rx_length)
{
	...
	offsetRx = DMA_OFFSET_ALIGN - (((unsigned int)&rdata_buf[0]) % DMA_OFFSET_ALIGN);
	offsetTx = DMA_OFFSET_ALIGN - (((unsigned int)&wdata_buf[0]) % DMA_OFFSET_ALIGN);

	memcpy(&wdata_buf[offsetTx], txbuf, tx_length);
	...
	, u8 command, u8 *data, u32 length)
{
	...
	if (res) {
		PRINTF(0, "(SJINU) fc8150_spi_dataread fail : %d\n", res);
		return BBM_NOK;
	}
...
int fc8150_spi_wordread(HANDLE hDevice, u16 addr, u16 *data)
{
	...
	mutex_unlock(&lock);
	PRINTF(0, "(SJINU) fc8150_spi_wordread : value=%x  res=%d\n", *data, res);
...
int fc8150_spi_wordwrite(HANDLE hDevice, u16 addr, u16 data)
{
	...
	mutex_unlock(&lock);
	PRINTF(0, "(SJINU) fc8150_spi_wordwrite : %d\n", res);
