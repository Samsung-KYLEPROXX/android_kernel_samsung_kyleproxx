...
extern struct device *sec_touchscreen;
					 ...
					 char *buf)
{
	...
	char fw_latest_version[10];
      ...
      /* for phone */
      sprintf(fw_latest_version, "CY%02X%04X", 
...
struct tsp_cmd tsp_cmds_cyttsp4[] = {
...
static int cyttsp4_core_probe(struct cyttsp4_core *core)
{
	...
	for (i = 0; i < ARRAY_SIZE(tsp_cmds_cyttsp4); i++)
	list_add_tail(&tsp_cmds_cyttsp4[i].list, &cd->cmd_list_head);
