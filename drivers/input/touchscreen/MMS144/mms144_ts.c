...
static int mms_ts_fw_load(struct mms_ts_info *info, bool force_config)
{
	...
	while (retries--) {
		...
		if (retries == 3)
			ret = mms100_ISC_download_mbinary(info->client,
				false);
		else
			ret = mms100_ISC_download_mbinary(info->client,
				true);
...
extern struct device *sec_touchkey;
...
static void fw_update(void *device_data)
{
	...
	case BUILT_IN:
		...
		disable_irq(info->irq);
#if 0
		ret = mms100_ISC_download_mbinary(info, false);
#else
