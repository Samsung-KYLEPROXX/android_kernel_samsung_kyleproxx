...
u32 *ist30xx_frame_rawbuf;
EXPORT_SYMBOL(ist30xx_frame_rawbuf);
			    ...
			    const char *buf, size_t size)
{
	...
	tsp_info("Use idle: %d, Use debug: %d\n", idle_en, debug_en);
	if (idle_en > 1 || debug_en > 1) {
		tsp_err("Unknown argument value, %d, %d\n",
			idle_en, debug_en);
...
/* sysfs: /sys/class/touch/internal/printk */
...
/* sysfs  */
static DEVICE_ATTR(mem, 0644, ist30xx_mem_show, ist30xx_mem_store);
static DEVICE_ATTR(refresh, 0644, ist30xx_frame_refresh, NULL);
static DEVICE_ATTR(filter, 0644, ist30xx_filter_show, NULL);
static DEVICE_ATTR(raw, 0644, ist30xx_raw_show, NULL);
static DEVICE_ATTR(base, 0644, ist30xx_base_show, NULL);
static DEVICE_ATTR(diff, 0644, ist30xx_diff_show, NULL);
static DEVICE_ATTR(clb, 0644, ist30xx_calib_show, ist30xx_calib_store);
static DEVICE_ATTR(debug, 0644, NULL, ist30xx_debug_store);
static DEVICE_ATTR(tsp_power, 0644, NULL, ist30xx_power_store);
static DEVICE_ATTR(errcnt, 0644, NULL, ist30xx_errcnt_store);
static DEVICE_ATTR(printk, 0644, ist30xx_printk_show, ist30xx_printk_store);

static DEVICE_ATTR(regcmd, 0644, tunes_regcmd_show, tunes_regcmd_store);
static DEVICE_ATTR(reg, 0644, tunes_reg_show, tunes_reg_store);
static DEVICE_ATTR(tunes_fw, 0644, NULL, tunes_fw_store);
static DEVICE_ATTR(tunes_param, 0644, NULL, tunes_param_store);
static DEVICE_ATTR(adb, 0644, tunes_adb_show, tunes_adb_store);
