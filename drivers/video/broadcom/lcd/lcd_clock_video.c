...
//		printk(KERN_ERR "Failed to enable the DSI AXI clock\n");
		return; 
	}
	if (clk_enable(dsi_esc)) {
//		printk(KERN_ERR "Failed to enable the DSI ESC clk\n");
		return;
	}
...
//		printk(KERN_ERR "Failed to enable the DSI PLL\n");
		return;
	}
	if (clk_enable(dsi_pll_ch)) {
//		printk(KERN_ERR "Failed to enable DSI PLL CH\n");
		return;
	}
//	printk("%s:%n\n", __func__, __LINE__);
        ...
        unsigned int esc_clk_hz )
{
	...
	if (g_display_enabled) {
		printk("Skipping brcm_enable_dsi_lcd_clocks\n");
		return 0;
	}
	printk("brcm_enable_dsi_lcd_clocks\n");
	if (pi_mgr_dfs_request_update(dfs_node, PI_OPP_TURBO))
	{
        ...
        unsigned int esc_clk_hz )
{
	...
	if (g_display_enabled) {
		printk("skipping brcm_enable_dsi_pll_clocks\n");
		return 0;
	}
	printk("brcm_enable_dsi_pll_clocks\n");
