...
static Int32 DSI_DCS_Read(DispDrv_PANEL_t *pPanel, UInt8 reg, UInt8 *rxBuff, UInt8 buffLen);
...
void panel_write(UInt8 *buff)
{
	DSI_ExecCmndList(panel, buff);
}


#define MAX_PARAMS_PER_PACKET 40

static void split_packet_and_send(DispDrv_PANEL_t *pPanel, u32 len, char *buff)
{
	int reg = *buff++;
	int param_cnt = len - 1;
	int pkt_cnt_p = (param_cnt + MAX_PARAMS_PER_PACKET - 1) / MAX_PARAMS_PER_PACKET;
			/* Len + Reg +param +param...		      Len + B0 + offset      Termination*/
	int exp_len = (pkt_cnt_p * (MAX_PARAMS_PER_PACKET + 1 + 1)) + (pkt_cnt_p * (2 + 1)) + 1;
	char *src = kmalloc(exp_len, GFP_KERNEL);
	char *dst = src;
	int sz;
	int offset = 0;
	int i;
	...
	for(i = 0; i < param_cnt; i++)
		printk("%x ", buff[i]);
	printk("exp len= %d src=0x%x pkt_cnt_p=%d param_cnt=%d reg=%x\n", exp_len, src,
				pkt_cnt_p, param_cnt, reg);
	len--; //to skip reg
	while (len) {
		*dst++ = 2; /*Size of B0 packet*/
		*dst++ = 0xB0;
		*dst++ = offset;
		if (len >= MAX_PARAMS_PER_PACKET)
			sz = MAX_PARAMS_PER_PACKET;
		else
			sz = len;
		*dst++ = sz + 1;
		*dst++ = reg;
		//copy len bytes from buf to dst
		memcpy(dst, buff, sz);
		dst += sz;
		buff += sz;
		offset += sz;
	...
	*dst = 0;
	...
	if (dst > (src + exp_len))
		printk("something went wrong with calculation\n");
	DSI_ExecCmndList(pPanel, src);
	kfree(src);
}
...
//			printk("sleep %d\n", *buff);
			...
			if (videoEnabled && (len > (MAX_PARAMS_PER_PACKET + 1))) {
				split_packet_and_send(pPanel, len, buff);
				goto done;
			}
#endif
			switch (len) {
	...
	DISPDRV_CB_T apiCb)
{
	...
	/* S6E63M0X3 panel require more dummy lines than actual image */
