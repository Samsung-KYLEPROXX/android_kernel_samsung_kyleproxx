...
static void touchkeyled_suspend(struct early_suspend *h)
{
	KLDBG("[KEYLED] %s, %d\n", __func__, __LINE__ );
}

static void touchkeyled_resume(struct early_suspend *h)
{
	KLDBG("[KEYLED] %s, %d\n", __func__, __LINE__ );
}
...
static void touchkeyled_shutdown(struct platform_device *pdev)
{
