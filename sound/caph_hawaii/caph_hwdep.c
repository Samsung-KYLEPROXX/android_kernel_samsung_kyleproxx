...
static int hwdep_open(struct snd_hwdep *hw, struct file *file)
{
	...
	if (rc)		
		return rc;
	aTrace(LOG_ALSA_INTERFACE, "ALSA-CAPH VoIP_Ioctl_Open %d:[%d,%d]\n",
		voipInstCnt, pVoIP->ulstarted, pVoIP->dlstarted);
	voipInstCnt = pVoIP->ulstarted + pVoIP->dlstarted;
	up(&sVoipAction);
...
static int hwdep_release(struct snd_hwdep *hw, struct file *file)
{
	...
	if (rc)		
		return rc;
	aTrace(LOG_ALSA_INTERFACE, "ALSA-CAPH VoIP_Ioctl_Release %d:[%d,%d]\n",
		voipInstCnt, pVoIP->ulstarted, pVoIP->dlstarted);
	voipInstCnt = pVoIP->ulstarted + pVoIP->dlstarted;
	up(&sVoipAction);
		       ...
		       unsigned int cmd, unsigned long arg)
{
	...
	case VoIP_Ioctl_Start:
		...
		if (voipInstCnt == 0) {	/* start VoIP only once */
			...
			if (pVoIP->buffer_handle)
				...
				voipInstCnt--;
				up(&sVoipAction);
				aTrace(LOG_ALSA_INTERFACE,"ENOMEM-1 voipInstCnt=%d\n", voipInstCnt);
				return -ENOMEM;
			}
			...
			if (pVoIP->buffer_handle->voip_data_dl_buf_ptr) {
				...
				voipInstCnt--;
				up(&sVoipAction);
				aTrace(LOG_ALSA_INTERFACE,"ENOMEM-2 voipInstCnt=%d\n", voipInstCnt);
				return -ENOMEM;
			}
			...
			if (pVoIP->buffer_handle->voip_data_ul_buf_ptr)
				...
				voipInstCnt--;
				up(&sVoipAction);
				aTrace(LOG_ALSA_INTERFACE,"ENOMEM-3 voipInstCnt=%d\n", voipInstCnt);
				return -ENOMEM;
			}
			...
			if (!pVoIP->buffer_handle->drv_handle) {
				...
				voipInstCnt--;
				up(&sVoipAction);
				aTrace(LOG_ALSA_INTERFACE,"ENOMEM-4 voipInstCnt=%d\n", voipInstCnt);
				return -ENOMEM;
			}
		...
		if (data == VoIP_UL)
			...
			pVoIP->dlstarted = 1;
			aTrace(LOG_ALSA_INTERFACE, "VoIP_Ioctl_Start ulstarted=%d dlstarted=%d",pVoIP->ulstarted, pVoIP->dlstarted);
		}
		up(&sVoipAction);
	...
	case VoIP_Ioctl_Stop:
		...
		{
			voipInstCnt--;
			aTrace(LOG_ALSA_INTERFACE, "VoIP_Ioctl_Stop--1--voipInstCnt=%d",voipInstCnt);
		}
			...
			voipInstCnt = 0;
			aTrace(LOG_ALSA_INTERFACE, "VoIP_Ioctl_Stop--2--voipInstCnt=%d",voipInstCnt);
			if (pVoIP->buffer_handle)
